//if someone called validation first name after call return the string value or null, only can pass string
export const validateFirstName = (name: string): string | null => {

    if (!name || name.trim().length === 0) {

        // if the name is empty and the length is 0
...
export const validateCountryCode = (countryCode: string): string | null => {
    ...
    //regex nad \ this is escape character
...
export const validateProfileImage = (
    ...
    //if image type is not empty check the types==
    ...
    if(image.fileSize && image.fileSize > 12 * 1024 * 1024){
